...
const getLike = async (req, res, next) => {
  let data = await db.query(
    "SELECT `goodshop`.`id`, `name`, `address`, `phone`, `lat`, `lng`, `info`, '1' AS `like` FROM `goodshop_like` LEFT JOIN `goodshop` ON `goodshop_like`.`goodshop_id`=`goodshop`.`id` WHERE `goodshop`.`id` IS NOT NULL AND `user_id`=?",
    [req.id]
  );
